...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if waiting == "set_origin_id":
        ...
        try:
            ...
            await update.message.reply_text(
                ...
                f"• Título: *{title}*\n"
                f"• Usuario: `{username}`\n\n"
                "Verifica que el bot sea administrador con permisos de lectura.",
    ...
    if waiting == "new_list_name":
        ...
        await update.message.reply_text("📋 Envía IDs separados por comas o líneas:", reply_markup=BACK_KB)
    ...
    # ── 3) Configurar mensajes reenviados ──
    ...
    if waiting == "edit_menu":
        ...
        if text=="🕒 Cambiar Intervalo":
            ...
            mens.pop(idx); mensajes_manager.save_mensajes(mens)
            await update.message.reply_text("✅ Mensaje eliminado.", reply_markup=MAIN_KB); context.user_data.pop("waiting_for")
    ...
    if waiting=="edit_choose_dest":
        ...
        if text=="👥 A Todos":
            ...
            await update.message.reply_text("✅ Se reenviará a todos.", reply_markup=MAIN_KB); context.user_data.pop("waiting_for")
        elif text=="📋 Lista":
            lists=list(cfg.get("listas_destinos",{}).keys())
            if not lists:
                await update.message.reply_text("⚠️ No hay listas.", reply_markup=MAIN_KB); context.user_data.pop("waiting_for")
    ...
    if text=="🗑️ Eliminar Mensaje" and not waiting:
        if not mens:
            await update.message.reply_text("⚠️ No hay mensajes para eliminar.", reply_markup=MAIN_KB); return
    ...
    if text=="🔁 Cambiar Intervalo" and not waiting:
        kb=ReplyKeyboardMarkup([
            ["🌐 Global","📄 Por Mensaje"],["📁 Por Lista"],["🔙 Volver"]
        ],resize_keyboard=True)
    ...
    if waiting=="change_zone":
        try:
            ...
            await update.message.reply_text(f"✅ Zona cambiada a `{text}`.", reply_markup=MAIN_KB)
        except:
